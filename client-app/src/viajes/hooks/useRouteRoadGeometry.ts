...
/**
 * Hook que consulta OSRM para obtener la ruta real por calles entre una serie de puntos.
 * Retorna la geometría (polyline), distancia total y duración estimada.
 */
export const useRouteRoadGeometry = () => {
  ...
  const fetchRoute = useCallback(async (waypoints: Waypoint[]) => {
    ...
    try {
      ...
      // Si falla OSRM, no mostramos geometría
      setGeometry(null);
    } finally {
      setIsLoading(false);
    }
  ...
  return { geometry, isLoading, fetchRoute };
};
