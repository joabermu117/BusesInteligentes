...
interface ActiveIncident {
  ...
  status: string;
  reportedAt?: string;
  incidentBuses?: Array<{ bus?: { plate?: string; id?: number } }>;
}

// ── Main Report Page ─────────────────────────────────────────
const ReporteTiempoReal = () => {
  ...
  // ── Active incidents from API ────────────────────────────────
  const [activeIncidents, setActiveIncidents] = useState<ActiveIncident[]>([]);

  useEffect(() => {
    httpClient.get(`${API_URL}/api/incidents`).then(({ data }) => {
      const active = (data as ActiveIncident[]).filter(
  ...
  const avgOccupancy = totalActiveBuses > 0
    ...
    : 0;
  const alertCount = busesWithIncidents + busesDelayed;
      ...
      >
          ...
          <Stack gap={1.25}>
            {activeBuses.slice(0, 10).map((bus) => (
              <Box key={bus.busId}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {bus.routeName ?? 'Sin ruta'} · {bus.currentStopName ?? '—'} · Ultima
                  actualizacion:{' '}
                  {bus.lastUpdate
        ...
        <Box>
          <Paper sx={{ p: 2.4, mb: 2 }}>
            ...
            {alerts.length === 0 && activeIncidents.length === 0 ? (
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                No hay alertas activas. Todo en orden.
              </Typography>
            ) : (
              <Stack gap={1} sx={{ mt: 1.4 }}>
                {/* Alertas de socket (retrasos, incidentes en vivo) */}
                ...
                {/* Incidentes activos desde la BD */}
                {activeIncidents.map((inc) => {
                  const busPlate = inc.incidentBuses?.[0]?.bus?.plate ?? '—';
                  ...
                  const severityColor = colorMap[inc.severity] || 'warning';
                  const typeLabels: Record<string, string> = {
                    mechanical: 'Falla mecanica',
                    accident: 'Accidente',
                    delay: 'Retraso',
                    passenger: 'Incidencia pasajero',
                    other: 'Otro',
                  };
                  return (
                    <Paper
                      ...
                      sx={{
                        ...
                        bgcolor: `${severityColor}.lighter`,
                      }}
                    >
                      <Typography variant="body2" fontWeight={600}>
                        {busPlate} · {typeLabels[inc.type] ?? inc.type}
                      </Typography>
          ...
          <Paper sx={{ p: 2.4 }}>
            ...
            <Stack gap={1.1} sx={{ mt: 1.8 }}>
              ...
              <Stack direction="row" justifyContent="space-between">
                <Typography variant="body2">Buses con ocupacion maxima</Typography>
              ...
              <Stack direction="row" justifyContent="space-between">
                <Typography variant="body2">Incidentes activos (BD)</Typography>
                <Typography variant="body2" sx={{ fontWeight: 700, color: 'error.main' }}>
...
export default ReporteTiempoReal;
